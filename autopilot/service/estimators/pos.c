...
void pos_update(pos_t *out, pos_in_t *in)
{
   ...
   /* run kalman filters: */
   kalman_run(&n_kalman,       &out->ne_pos.n,    &out->ne_speed.n,    in->pos_n,   in->speed_e, in->acc.n, in->dt);
   kalman_run(&e_kalman,       &out->ne_pos.e,    &out->ne_speed.e,    in->pos_e,   in->speed_n, in->acc.e, in->dt);
